...
A structured method for building that library is the Domain-to-Tasks-to-Skills-to-Automation progression. Start by decomposing the system's responsibility into distinct domains — bounded areas of concern such as data ingestion, report generation, or customer communication. For each domain, enumerate the recurring tasks that the agent must handle: not edge cases, but the repeatable operations that occur weekly or more. Each recurring task then maps to a skill definition: a named, scoped procedure with a clear input contract, a defined output format, and explicit quality criteria. Finally, each candidate skill is assessed for automation potential by evaluating its input predictability, the verifiability of its outputs, and the cost of an incorrect result. Skills that score well on all three become first-class entries in the library; those that do not are flagged as human-in-the-loop steps or deferred.
