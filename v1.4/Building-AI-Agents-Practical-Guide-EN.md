...
**The minimal working harness.** In the smallest working setup the four roles collapse into three concrete artifacts; a production design must still account for all four, but the minimal version is a useful mental anchor for what actually runs: a while-loop that continues until the task is done or a step limit is reached, a single read-file tool so the agent can consult external content, and a plain text file used as scratch memory. That three-part arrangement already exhibits the key harness properties: it loops, it has tool access, and it maintains state outside the context window. Error handling is implicit at this scale, handled by the loop's exit condition; in a production design it becomes the fourth explicit part. Everything in production is an elaboration of this skeleton.

**The harness as a closing bracket.** Sections 3.1 through 3.5 identified the four building blocks individually. The harness is the structure in which they converge. The planning tool (3.1) runs inside the loop and writes its output to the file system (3.3). Sub-agents (3.2) are spawned by the same loop with isolated context. The detailed prompter (3.4) governs the loop's decision logic. Without the harness, the four components remain independent; with it, they form a single operating cycle.
...
A structured method for building that library is the Domain-to-Tasks-to-Skills-to-Automation progression. Start by decomposing the system's responsibility into distinct domains (bounded areas of concern such as data ingestion, report generation, or customer communication). For each domain, enumerate the recurring tasks that the agent must handle: not edge cases, but the repeatable operations that occur weekly or more frequently. Each recurring task then maps to a skill definition: a named, scoped procedure with a clear input contract, a defined output format, and explicit quality criteria. Finally, each candidate skill is assessed for automation potential by evaluating its input predictability, the verifiability of its outputs, and the cost of an incorrect result. Skills that score well on all three become first-class entries in the library; those that do not are flagged as human-in-the-loop steps or deferred.

Applying this progression domain by domain produces intentional coverage rather than the reactive accumulation that characterizes ad-hoc prompting. The audit trail is a byproduct: because every skill traces back to a domain and a recurring task, gaps in coverage become visible and prioritization becomes data-driven. For engineering teams maintaining agent systems over time, the progression also makes onboarding tractable. New engineers can read the domain map, understand which tasks each skill handles, and add or modify skills without reverse-engineering a single large prompt.
...
- [ ] SSE `AbortSignal` propagated end-to-end; session lifecycle explicit.
...
- [ ] API gateway: auth endpoints, session management, SSE streaming -- tokens validated at the gateway; session lifecycle (creation, refresh, revocation) explicit; SSE connections propagate `AbortSignal`.
