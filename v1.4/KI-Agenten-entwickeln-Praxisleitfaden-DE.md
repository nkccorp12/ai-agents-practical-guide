...
**Protocol.** Die Protocol-Schicht standardisiert die Verbindung der Runtime zu externen Fähigkeiten. Das Model Context Protocol (MCP) ist der aufkommende Standard, der maßgeschneiderte Einzelintegrationen durch einen einheitlichen Tool-Anbindungsvertrag ersetzt. Kapitel 4 behandelt die auf diesem Protokoll aufbauende Skills-Layer-Architektur.

**Tools und Sandboxes.** Werkzeuge geben Agenten Reichweite: Code-Ausführung, Browser-Steuerung, Dateisystemzugriff, externe API-Aufrufe. Die Sandbox-Schicht kapselt jedes Werkzeug mit Ausführungsgrenzen, Rate-Limits und Berechtigungsprüfungen, die verhindern, dass ein fehlerhaftes Werkzeug das Gesamtsystem kompromittiert. Kapitel 11 behandelt die Sicherheitsarchitektur dieser Schicht.
...
**Warum sich dasselbe Modell in verschiedenen Produkten anders anfühlt.** Claude Code, Cursor und Codex laufen alle auf denselben Basismodellen. Ihr Verhalten und ihre Ausgaben fühlen sich für Anwender dennoch deutlich unterschiedlich an. Der Unterschied liegt nahezu vollständig im Harness: welche Tools verfügbar sind und wie sie beschrieben werden, wie viel der Konversationshistorie erhalten oder zusammengefasst wird, zu welchem Zeitpunkt Dateiinhalt in den Kontext injiziert wird, wie Fehler wiederholt oder gemeldet werden und wie der System-Prompt die Rolle des Modells rahmt. Zwei Agenten, die auf demselben Modell laufen, aber unterschiedliche Harness-Designs haben, sind aus praktischer Sicht verschiedene Produkte.

**Das minimale funktionierende Harness.** Im kleinsten funktionierenden Setup fallen die vier Rollen in drei konkrete Artefakte zusammen; ein Produktionsdesign muss dennoch alle vier bedenken, doch die minimale Variante ist ein nützlicher Referenzpunkt für das, was tatsächlich läuft: eine While-Schleife, die weiterläuft, bis die Aufgabe abgeschlossen ist oder ein Schrittelimit erreicht wird, ein einzelnes Read-File-Tool, damit der Agent externe Inhalte konsultieren kann, und eine einfache Textdatei als Scratch-Memory. Diese dreiteilige Anordnung zeigt bereits die wesentlichen Harness-Eigenschaften: Sie hat einen Loop, sie hat Tool-Zugriff, und sie verwaltet Zustand außerhalb des Kontextfensters. Fehlerbehandlung ist auf dieser Ebene implizit, getragen von der Abbruchbedingung des Loops; in einem Produktionsdesign wird sie zum vierten expliziten Bestandteil. Alles in Produktionssystemen ist eine Ausarbeitung dieses Grundgerüsts.

**Das Harness als schließende Klammer.** Die Abschnitte 3.1 bis 3.5 haben die vier Bausteine einzeln beschrieben. Das Harness ist die Struktur, in der sie zusammenlaufen. Das Planungstool (3.1) läuft innerhalb des Loops und schreibt seine Ausgabe ins File-System (3.3). Sub-Agents (3.2) werden vom selben Loop mit isoliertem Kontext gestartet. Der detaillierte Prompt-Aufbau (3.4) steuert die Entscheidungslogik des Loops. Ohne das Harness bleiben die vier Komponenten unverbunden; mit ihm bilden sie einen einzigen Betriebszyklus.
...
Eine strukturierte Methode für den Aufbau dieser Bibliothek ist die Schritt-für-Schritt-Methode Domain-to-Tasks-to-Skills-to-Automation. Der erste Schritt ist die Zerlegung der Systemzuständigkeit in klar abgegrenzte Domänen, also in thematisch zusammengehörige Zuständigkeitsbereiche wie Datenaufnahme, Berichterstellung oder Kundenkommunikation. Für jede Domäne werden die wiederkehrenden Tasks aufgelistet, die der Agent regelmäßig erledigen muss: nicht Sonderfälle, sondern Operationen, die wöchentlich oder häufiger anfallen. Jeder dieser Tasks wird anschließend in eine Skill-Definition überführt: einen benannten, klar abgegrenzten Ablauf mit definiertem Input-Vertrag, festgelegtem Output-Format und expliziten Qualitätskriterien. Abschließend wird jeder Skill-Kandidat auf sein Automatisierungspotenzial geprüft, indem die Vorhersagbarkeit der Eingaben, die Überprüfbarkeit der Ausgaben und die Kosten eines fehlerhaften Ergebnisses bewertet werden. Skills mit hohem Potenzial auf allen drei Dimensionen werden als vollwertige Bibliothekseinträge aufgenommen; die übrigen werden als Human-in-the-Loop-Schritte markiert oder zurückgestellt.

Die domänenweise Anwendung dieser Progression erzeugt eine absichtsvolle Abdeckung statt der reaktiven Anhäufung, die Ad-hoc-Prompting kennzeichnet. Der Audit-Trail entsteht als Nebenprodukt: Da jeder Skill auf eine Domäne und einen wiederkehrenden Task zurückverweist, werden Lücken in der Abdeckung sichtbar, und die Priorisierung wird datengetrieben. Für Engineering-Teams, die Agentensysteme langfristig betreiben, macht die Progression auch das Onboarding handhabbar: Neue Entwicklerinnen und Entwickler können die Domänenkarte lesen, verstehen, welche Tasks jeder Skill abdeckt, und Skills hinzufügen oder ändern, ohne einen einzelnen großen Prompt rückwärts zu analysieren.
...
**Inter-Agenten-Misinformation.** Eine dreistufige Pipeline vervielfacht die Retrieval-Möglichkeiten, aber auch die Retrieval-Fehlerpunkte. Wenn eine schwache Retrieval-Stufe in einem frühen Agenten Ergebnisse schlechter Qualität produziert und diese als scheinbar verlässliche Ausgabe weitergibt, synthetisieren nachfolgende Agenten aus diesem Rauschen statt aus relevantem Material. Der Fehler bleibt nicht lokal: Er propagiert durch jeden Hop und erreicht die abschließende Antwort verstärkt, nicht korrigiert. Dieses Muster unterscheidet sich von Einzelagenten-Halluzinationen und verdient eine eigene Bezeichnung: Inter-Agenten-Misinformation oder Downstream-Loss.

Eine naheliegende Reaktion ist, das Modell in der Synthesestufe aufzuwerten. Das löst das Problem nicht. Ein leistungsfähigeres Modell, das eine schwache Retrieval-Ausgabe erhält, produziert überzeugendere Halluzinationen, keine genaueren Antworten. Der Defekt liegt im Retrieval-Signal, nicht in der Generierungskapazität.

Vier Maßnahmen adressieren das Problem auf Architekturebene. Erstens: Retrieval-Qualität an jedem Hop validieren, nicht nur am Eintrittspunkt. Jede Agenten-zu-Agenten-Übergabe sollte ein Relevanzsignal zusammen mit dem abgerufenen Inhalt enthalten. Zweitens: harte Relevanz-Schwellen anlegen. Liegen die Top-k-Ergebnisse eines Retrieval-Schritts unterhalb eines Mindestscore, darf der Agent sie nicht als valide Evidenz behandeln. Drittens: einen formalen Rückgabestatus "Informationen unzureichend" definieren. Wenn der abgerufene Kontext den Schwellenwert nicht erfüllt, gibt der Agent diesen Status zurück, statt aus marginalen Ergebnissen zu synthetisieren. Nachgelagerte Agenten, die diesen Status erhalten, können eskalieren, auf eine breitere Suche zurückfallen oder die Lücke explizit an den Nutzer melden. Viertens: jede Agenten-Retrieval-Schnittstelle bei Entwurf und Evaluation als eigenständige Risikooberfläche behandeln. Eine Integration, die isoliert korrekt erscheint, kann unter realen Abfrageverteilungen stillschweigend fehlerhafte Ergebnisse propagieren.

Die in Abschnitt 8.6 beschriebene Re-Ranking-Stufe ist komplementär: Re-Ranking verbessert die Reihenfolge einer abgerufenen Menge, die die Relevanzhürde bereits erreicht, kann aber einen Retrieval-Schritt nicht retten, der von Anfang an die falschen Dokumente zurückgegeben hat. Evaluierungs-Gates für Multi-Agenten-Pipelines werden in Kapitel 9 behandelt.
...
Kapitel 9 baut das Eval-System: Testfälle werden entworfen, der Eval-Harness verdrahtet und vor jedem Deployment ausgeführt. Abschnitt 12.9 beschreibt, was nach dem Deployment passiert -- der operative Feedback-Kreislauf eines laufenden Prod-Systems. Produktions-Traffic bringt Fehlermodi an die Oberfläche, die kein Vorab-Test vollständig abdeckt; der Großteil der realen Edge Cases wird erstmals von echten Nutzern unter echten Bedingungen entdeckt.
...
### Production-Readiness-Schichten

Kompakte Checkliste der neun Production-Readiness-Schichten aus Abschnitt 12.8. Details siehe Abschnitt 12.8.

- [ ] Modulare Codebase und Config-Hygiene -- keine Secrets oder Debug-Flags im Production-Build; umgebungsspezifische Konfiguration ueber Umgebungsvariablen.
- [ ] Datensicherheit: DTOs -- Roh-Modelle erreichen den Client nicht; jede API-Antwort wird ueber ein Data Transfer Object geformt.
- [ ] Security: Rate-Limiting, Input-Sanitization, Authentifizierung -- alle Endpunkte geschuetzt; Nutzereingaben vor Prompt-Interpolation sanitisiert; per-IP- und per-Tenant-Rate-Limits am Gateway.
- [ ] Service-Layer: Connection-Pooling, Retries mit exponentiellem Backoff, Modell-Tier-Fallback -- Pools mit expliziten Groessenlimits konfiguriert; wiederholbare Fehler mit Backoff und Jitter wiederholt; Komplexitaets-Klassifikator leitet an guenstigstes Tier weiter.
...
- [ ] Eval-Framework mit LLM-as-Judge -- Regressions-Eval-Suite deckt zentrale User-Journeys ab; mindestens ein automatisierter Judge-Prompt bewertet Ausgabequalitaet vor jedem Deployment.
- [ ] Stress-Testing unter realistischer Last -- Lasttest unter erwarteter Peak-Concurrency durchgefuehrt; Bottlenecks behoben; Graceful-Degradation (Queuing, Throttling, verstaendliche Fehlermeldungen) verifiziert.
